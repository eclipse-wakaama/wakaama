...
uint8_t lwm2m_setAttributes(lwm2m_context_t * contextP, lwm2m_uri_t * uriP, lwm2m_attribute_type_t attrType,
		const char* value, lwm2m_object_t* objectP, lwm2m_server_t * serverP)
{
	//-------------------------------------------------------------------- JH --
	...
	if (LWM2M_URI_IS_SET_RESOURCE(uriP))
	{
		...
		case ATTRIBUTE_MAX_PERIOD: {
			...
			attributeP->nextTransmission = tv.tv_sec + strtol(valStr, NULL, 10);
		}
			break;
		case ATTRIBUTE_GREATER_THEN:
...
bool *notifyResult)
{
	...
	*notifyResult = false;
	...
	{ // check if max period is elapsed
		int maxPer = strtol(attrData->maxPeriod, NULL, 10);
	...
	if (false == *notifyResult)
	{
		...
		case LWM2M_DATATYPE_STRING: { // lexigraphic order less & greater !
			...
			{
				cmp = (strcmp(resValStr, attrData->lessThan) < 0 && strcmp(resValStr, attrData->greaterThan) > 0);
			}
		...
		case LWM2M_DATATYPE_INTEGER: {
			...
			if (sscanf(resValStr, "%ld", &value) == 0)
				break;
			if (attrData->greaterThan != NULL)
			{
				if (sscanf(attrData->greaterThan, "%ld", &gt) == 0)
					break;
			}
			if (attrData->lessThan != NULL)
			{
				if (sscanf(attrData->lessThan, "%ld", &lt) == 0)
					break;
			}
			if (attrData->step != NULL && attrData->oldValue != NULL)
			{
				if (sscanf(attrData->step, "%ld", &st) == 0)
					break;
				if (sscanf(attrData->oldValue, "%ld", &ov) == 0)
					break;
			}
			if (attrData->lessThan != NULL && attrData->greaterThan != NULL)
			{
		...
		case LWM2M_DATATYPE_FLOAT: {
			...
			if (sscanf(resValStr, "%lf", &value) == 0)
				break;
			if (attrData->greaterThan != NULL)
			{
				if (sscanf(attrData->greaterThan, "%lf", &gt) == 0)
					break;
			}
			if (attrData->lessThan != NULL)
			{
				if (sscanf(attrData->lessThan, "%lf", &lt) == 0)
					break;
			}
			if (attrData->step != NULL && attrData->oldValue != NULL)
			{
				if (sscanf(attrData->step, "%lf", &st) == 0)
					break;
				if (sscanf(attrData->oldValue, "%lf", &ov) == 0)
					break;
			}
			if (attrData->lessThan != NULL && attrData->greaterThan != NULL)
			{
	...
	if (ret != COAP_NO_ERROR || false == *notifyResult)
	{
