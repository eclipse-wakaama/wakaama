...
static int prv_get_cbor_type_and_value(const uint8_t *buffer,
                                       size_t bufferLen,
                                       cbor_type_t * cborTypeP,
                                       uint64_t * valP)
{
    int res;
    int result = 0;
    ...
    {
        // Just ignore the tag
        res = cbor_get_type_and_value(buffer + result,
                                      bufferLen - result,
                                      cborTypeP,
                                      valP);
        if (res <= 0) return res;
                         ...
                         lwm2m_data_t *baseValue)
{
    ...
    if (res <= 0) return -1;
    offset += res;
    if (cborType != CBOR_TYPE_MAP) return -1;
    count = (int)val;
    if (((uint64_t)count) != val || count <= 0 || count > (int)(bufferLen / 2)) return -1;
    ...
    for (; count > 0; count--)
    {
        ...
        res = cbor_get_singular(buffer + offset,
                                bufferLen - offset,
                                &data);
        if (res <= 0) return -1;
        offset += res;
        ...
        {
            if (data.value.asBuffer.length == 3
             && memcmp(data.value.asBuffer.buffer, "vlo", 3) == 0)
            {
            ...
            else
            {
                ...
                if (data.value.asBuffer.buffer[data.value.asBuffer.length - 1] == '_') return -1;
                label = SENML_CBOR_UNKNOWN_LABEL;
            }
        }
        else if (lwm2m_data_decode_int(&data, &ival) != 0)
        {
            if (ival >= SENML_CBOR_BASE_SUM_LABEL
             && ival <= SENML_CBOR_DATA_VALUE_LABEL)
            {
                label = (int)ival;
            }
            else
        ...
        case SENML_CBOR_BASE_TIME_LABEL:
            if (btSeen) return -1;
            btSeen = true;
            if (lwm2m_data_decode_int(&data, &ival) == 0) return -1;
            *baseTime = (time_t)ival;
            break;
        case SENML_CBOR_BASE_NAME_LABEL:
            if (bnSeen) return -1;
            bnSeen = true;
            if (data.type != LWM2M_TYPE_STRING) return -1;
            if (data.value.asBuffer.length > 0)
            {
                if (data.value.asBuffer.length == 1
                 && data.value.asBuffer.buffer[0] != '/') return -1;
                if (data.value.asBuffer.length > URI_MAX_STRING_LEN) return -1;
        ...
        case SENML_CBOR_BOOLEAN_VALUE_LABEL:
            {
                bool bval;
                if (recordP->value.type != LWM2M_TYPE_UNDEFINED) return -1;
                if (lwm2m_data_decode_bool(&data, &bval) == 0) return -1;
        ...
        case SENML_CBOR_DATA_VALUE_LABEL:
            if (recordP->value.type != LWM2M_TYPE_UNDEFINED) return -1;
            if (data.type != LWM2M_TYPE_OPAQUE) return -1;
        ...
        case SENML_CBOR_OBJECT_LINK_LABEL:
            if (recordP->value.type != LWM2M_TYPE_UNDEFINED) return -1;
            if (data.type != LWM2M_TYPE_STRING) return -1;
            if (!utils_textToObjLink(data.value.asBuffer.buffer,
...
static bool prv_convertValue(const senml_record_t * recordP,
                             lwm2m_data_t * targetP)
{
    ...
    case LWM2M_TYPE_OPAQUE:
        lwm2m_data_encode_opaque(recordP->value.value.asBuffer.buffer,
                                 recordP->value.value.asBuffer.length,
                                 targetP);
        break;
    default:
...
int senml_cbor_parse(const lwm2m_uri_t * uriP, const uint8_t * buffer, size_t bufferLen, lwm2m_data_t ** dataP)
{
    ...
    *dataP = NULL;
    ...
    if (res <= 0) goto error;
    offset += res;
    if (cborType != CBOR_TYPE_ARRAY) goto error;
    count = (int)val;
    if (((uint64_t)count) != val || count <= 0 || ((size_t)count) > bufferLen / 3) goto error;
    ...
    {
        res = prv_parseItem(buffer + offset,
                            bufferLen - offset,
    ...
    if (offset != bufferLen) goto error;
...
static int prv_serializeBaseName(const uint8_t * baseUriStr,
                                 size_t baseUriLen,
                                 uint8_t * buffer,
                                 size_t bufferLen)
{
    ...
    // value
    res = cbor_put_singular(buffer + head,
                            bufferLen - head,
                            &data);
    if (res <= 0) return -1;
    return head + res;
                             ...
                             size_t bufferLen)
{
    ...
    if (parentUriLen > 0)
    {
        if (sizeof(uriStr) < parentUriLen) return -1;
                            ...
                            size_t bufferLen)
{
    ...
    res = 0;
    if (!*baseNameOutput && baseUriLen > 0) res++;
    if (!baseUriLen || level > baseLevel) res++;
    switch (tlvP->type)
    {
    ...
    {
        res = prv_serializeBaseName(baseUriStr,
                                    baseUriLen,
                                    buffer + head,
                                    bufferLen - head);
        if (res <= 0) return -1;
    ...
    case LWM2M_TYPE_OBJECT_INSTANCE:
    {
        ...
        else
        {
            ...
            res = utils_intToText(tlvP->id,
                                  uriStr + uriLen,
                                  sizeof(uriStr) - uriLen);
            if (res <= 0) return -1;
            uriLen += res;
            if (uriLen >= sizeof(uriStr)) return -1;
            uriStr[uriLen++] = '/';
...
int senml_cbor_serialize(const lwm2m_uri_t * uriP, int size, const lwm2m_data_t * tlvP, uint8_t ** bufferP)
{
    ...
    if (!baseNameOutput && baseUriLen > 0)
    {
        ...
        // Start the record
        res = cbor_put_type_and_value(bufferCBOR + head,
                                      PRV_CBOR_BUFFER_SIZE - head,
                                      CBOR_TYPE_MAP,
                                      1);
        if (res < 0) return res;
