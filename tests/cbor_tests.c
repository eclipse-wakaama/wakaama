...
static void cbor_test_raw_error(const char * uriStr,
 ...
 *        to original.
 */
static void cbor_test_data_expected(const char * uriStr,
                     lwm2m_data_t * dataP,
        ...
        else
        {
            ...
            case LWM2M_TYPE_OPAQUE:
                if (tlvP->type != dataP->type
                 || tlvP->value.asBuffer.length != dataP->value.asBuffer.length
                 || memcmp(tlvP->value.asBuffer.buffer, dataP->value.asBuffer.buffer, tlvP->value.asBuffer.length) != 0)
                {
                    printf("(Parsing %s from %s differs from original.)\t", id, STR_MEDIA_TYPE(format));
                    CU_TEST(CU_FALSE);
                }
            ...
            case LWM2M_TYPE_INTEGER:
                {
                    int64_t v1, v2;
                    if (lwm2m_data_decode_int(dataP, &v1) == 0
                     || lwm2m_data_decode_int(tlvP, &v2) == 0
                     || v1 != v2)
                    {
                        printf("(Parsing %s from %s differs from original.)\t", id, STR_MEDIA_TYPE(format));
                        CU_TEST(CU_FALSE);
            ...
            {
                uint64_t v1, v2;
                if (lwm2m_data_decode_uint(dataP, &v1) == 0
                 || lwm2m_data_decode_uint(tlvP, &v2) == 0
                 || v1 != v2)
                {
                    printf("(Parsing %s from %s differs from original.)\t", id, STR_MEDIA_TYPE(format));
                    CU_TEST(CU_FALSE);
            ...
            case LWM2M_TYPE_BOOLEAN:
                if (tlvP->type != dataP->type
                 || tlvP->value.asBoolean != dataP->value.asBoolean)
                {
                    printf("(Parsing %s from %s differs from original.)\t", id, STR_MEDIA_TYPE(format));
                    CU_TEST(CU_FALSE);
                }
            ...
            case LWM2M_TYPE_OBJECT_LINK:
                if (tlvP->type != dataP->type
                 || tlvP->value.asObjLink.objectId != dataP->value.asObjLink.objectId
            ...
            case LWM2M_TYPE_CORE_LINK:
                ...
                if (tlvP->type != LWM2M_TYPE_STRING
                 || tlvP->value.asBuffer.length != dataP->value.asBuffer.length
                 || memcmp(tlvP->value.asBuffer.buffer, dataP->value.asBuffer.buffer, tlvP->value.asBuffer.length) != 0)
                {
                    printf("(Parsing %s from %s differs from original.)\t", id, STR_MEDIA_TYPE(format));
...
{
    lwm2m_data_t data;
    const uint8_t expected[] = { 0x1b, 0x00, 0x00, 0x00, 0xe8, 0xd4, 0xa5, 0x10,
                                 0x00 };
    memset(&data, 0, sizeof(data));
    data.type = LWM2M_TYPE_INTEGER;
...
{
    lwm2m_data_t data;
    const uint8_t expected[] = { 0x1b, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                                 0xff };
    memset(&data, 0, sizeof(data));
...
{
    lwm2m_data_t data;
    const uint8_t expected[] = { 0x3b, 0x7f, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                                 0xff };
    memset(&data, 0, sizeof(data));
    data.type = LWM2M_TYPE_INTEGER;
...
{
    lwm2m_data_t data;
    const uint8_t expected[] = { 0x1b, 0x7f, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                                 0xff };
    memset(&data, 0, sizeof(data));
    data.type = LWM2M_TYPE_INTEGER;
...
{
    lwm2m_data_t data;
    const uint8_t expected[] = { 0xfb, 0x3f, 0xf1, 0x99, 0x99, 0x99, 0x99, 0x99,
                                 0x9a };
    memset(&data, 0, sizeof(data));
    data.type = LWM2M_TYPE_FLOAT;
...
{
    lwm2m_data_t data;
    const uint8_t expected[] = { 0xfb, 0x7e, 0x37, 0xe4, 0x3c, 0x88, 0x00, 0x75,
                                 0x9c };
    memset(&data, 0, sizeof(data));
    data.type = LWM2M_TYPE_FLOAT;
...
{
    lwm2m_data_t data;
    const uint8_t expected[] = { 0xfb, 0xc0, 0x10, 0x66, 0x66, 0x66, 0x66, 0x66,
                                 0x66 };
    memset(&data, 0, sizeof(data));
    data.type = LWM2M_TYPE_FLOAT;
...
{
    lwm2m_data_t data;
    const uint8_t expected[] = { 0x74, 0x32, 0x30, 0x31, 0x33, 0x2d, 0x30, 0x33,
                                 0x2d, 0x32, 0x31, 0x54, 0x32, 0x30, 0x3a, 0x30,
...
{
    lwm2m_data_t data;
    const uint8_t expected[] = { 0xfb, 0x41, 0xd4, 0x52, 0xd9, 0xec, 0x20, 0x00,
                                 0x00 };
    memset(&data, 0, sizeof(data));
    data.type = LWM2M_TYPE_FLOAT;
...
{
    lwm2m_data_t data;
    const uint8_t expected[] = { 0x76, 0x68, 0x74, 0x74, 0x70, 0x3a, 0x2f, 0x2f,
...
static void cbor_test_47(void)
{
    ...
    const uint8_t buffer[] = { 0x3b, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                               0xff };
...
{
    // {"a": 1, "b": [2, 3]}
    const uint8_t buffer[] = { 0xa2, 0x61, 0x61, 0x01, 0x61, 0x62, 0x82, 0x02,
                               0x03 };
...
{
    // {"a": "A", "b": "B", "c": "C", "d": "D", "e": "E"}
    const uint8_t buffer[] = { 0xa5, 0x61, 0x61, 0x61, 0x41, 0x61, 0x62, 0x61,
...
{
    // (_ h’0102’, h’030405’)
    const uint8_t buffer[] = { 0x5f, 0x42, 0x01, 0x02, 0x43, 0x03, 0x04, 0x05,
                               0xff };
...
{
    // (_ "strea", "ming")
    const uint8_t buffer[] = { 0x7f, 0x65, 0x73, 0x74, 0x72, 0x65, 0x61, 0x64,
...
{
    // [_ 1, [2, 3], [_ 4, 5]]
    const uint8_t buffer[] = { 0x9f, 0x01, 0x82, 0x02, 0x03, 0x9f, 0x04, 0x05,
                               0xff, 0xff };
...
{
    // [_ 1, [2, 3], [4, 5]]
    const uint8_t buffer[] = { 0x9f, 0x01, 0x82, 0x02, 0x03, 0x82, 0x04, 0x05,
                               0xff };
...
{
    // [1, [2, 3], [_ 4, 5]]
    const uint8_t buffer[] = { 0x83, 0x01, 0x82, 0x02, 0x03, 0x9f, 0x04, 0x05,
                               0xff };
...
{
    // [1, [_ 2, 3], [4, 5]]
    const uint8_t buffer[] = { 0x83, 0x01, 0x9f, 0x02, 0x03, 0xff, 0x82, 0x04,
                               0x05 };
...
static void cbor_test_67(void)
{
    ...
    // 20, 21, 22, 23, 24, 25]
    const uint8_t buffer[] = { 0x9f, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
...
{
    // {_ "a": 1, "b": [_ 2, 3]}
    const uint8_t buffer[] = { 0xbf, 0x61, 0x61, 0x01, 0x61, 0x62, 0x9f, 0x02,
                               0x03, 0xff, 0xff };
...
{
    // ["a", {_ "b": "c"}]
    const uint8_t buffer[] = { 0x82, 0x61, 0x61, 0xbf, 0x61, 0x62, 0x61, 0x63,
                               0xff };
...
{
    // {_ "Fun": true, "Amt": -2}
    const uint8_t buffer[] = { 0xbf, 0x63, 0x46, 0x75, 0x6e, 0xf5, 0x63, 0x41,
                               0x6d, 0x74, 0x21, 0xff };
...
{
    // Infinity
    const uint8_t buffer[] = { 0xfb, 0x7f, 0xf0, 0x00, 0x00, 0x00, 0x00, 0x00,
                               0x00 };
    lwm2m_uri_t uri;
    int size;
    lwm2m_data_t * tlvP;
...
{
    // NaN
    const uint8_t buffer[] = { 0xfb, 0x7f, 0xf8, 0x00, 0x00, 0x00, 0x00, 0x00,
                               0x00 };
    lwm2m_uri_t uri;
    int size;
    lwm2m_data_t * tlvP;
...
{
    // -Infinity
    const uint8_t buffer[] = { 0xfb, 0xff, 0xf0, 0x00, 0x00, 0x00, 0x00, 0x00,
                               0x00 };
    lwm2m_uri_t uri;
    int size;
    lwm2m_data_t * tlvP;
...
static void cbor_test_77(void)
{
    ...
    const uint8_t buffer[] = { 0xc0, 0x74, 0x32, 0x30, 0x31, 0x33, 0x2d, 0x30,
    ...
    CU_ASSERT_EQUAL_FATAL(memcmp(tlvP->value.asBuffer.buffer,
...
{
    // 1(1363896240.5)
    const uint8_t buffer[] = { 0xc1, 0xfb, 0x41, 0xd4, 0x52, 0xd9, 0xec, 0x20,
                               0x00, 0x00 };
    lwm2m_uri_t uri;
    int size;
    lwm2m_data_t * tlvP;
...
static void cbor_test_82(void)
{
    ...
    const uint8_t buffer[] = { 0xd8, 0x20, 0x76, 0x68, 0x74, 0x74, 0x70, 0x3a,
...
static void cbor_test_83(void)
{
    ...
    const uint8_t buffer[] = { 0xd8, 0x20, 0xd8, 0x20, 0x76, 0x68, 0x74, 0x74,
...
static struct TestTable table[] = {
        { "test of cbor_test_1()", cbor_test_1 },
        { "test of cbor_test_2()", cbor_test_2 },
        { "test of cbor_test_3()", cbor_test_3 },
        { "test of cbor_test_4()", cbor_test_4 },
        { "test of cbor_test_5()", cbor_test_5 },
        { "test of cbor_test_6()", cbor_test_6 },
        { "test of cbor_test_7()", cbor_test_7 },
        { "test of cbor_test_8()", cbor_test_8 },
        { "test of cbor_test_9()", cbor_test_9 },
        { "test of cbor_test_10()", cbor_test_10 },
        { "test of cbor_test_11()", cbor_test_11 },
        { "test of cbor_test_12()", cbor_test_12 },
        { "test of cbor_test_13()", cbor_test_13 },
        { "test of cbor_test_14()", cbor_test_14 },
        { "test of cbor_test_15()", cbor_test_15 },
        { "test of cbor_test_16()", cbor_test_16 },
        { "test of cbor_test_17()", cbor_test_17 },
        { "test of cbor_test_18()", cbor_test_18 },
        { "test of cbor_test_19()", cbor_test_19 },
        { "test of cbor_test_20()", cbor_test_20 },
        { "test of cbor_test_21()", cbor_test_21 },
        { "test of cbor_test_22()", cbor_test_22 },
        { "test of cbor_test_23()", cbor_test_23 },
        { "test of cbor_test_24()", cbor_test_24 },
        { "test of cbor_test_25()", cbor_test_25 },
        { "test of cbor_test_26()", cbor_test_26 },
        { "test of cbor_test_27()", cbor_test_27 },
        { "test of cbor_test_28()", cbor_test_28 },
        { "test of cbor_test_29()", cbor_test_29 },
        { "test of cbor_test_30()", cbor_test_30 },
        { "test of cbor_test_31()", cbor_test_31 },
        { "test of cbor_test_32()", cbor_test_32 },
        { "test of cbor_test_33()", cbor_test_33 },
        { "test of cbor_test_34()", cbor_test_34 },
        { "test of cbor_test_35()", cbor_test_35 },
        { "test of cbor_test_36()", cbor_test_36 },
        { "test of cbor_test_37()", cbor_test_37 },
        { "test of cbor_test_38()", cbor_test_38 },
        { "test of cbor_test_39()", cbor_test_39 },
        { "test of cbor_test_40()", cbor_test_40 },
        { "test of cbor_test_41()", cbor_test_41 },
        { "test of cbor_test_42()", cbor_test_42 },
        { "test of cbor_test_43()", cbor_test_43 },
        { "test of cbor_test_44()", cbor_test_44 },
        { "test of cbor_test_45()", cbor_test_45 },
        { "test of cbor_test_46()", cbor_test_46 },
        { "test of cbor_test_47()", cbor_test_47 },
        { "test of cbor_test_48()", cbor_test_48 },
        { "test of cbor_test_49()", cbor_test_49 },
        { "test of cbor_test_50()", cbor_test_50 },
        { "test of cbor_test_51()", cbor_test_51 },
        { "test of cbor_test_52()", cbor_test_52 },
        { "test of cbor_test_53()", cbor_test_53 },
        { "test of cbor_test_54()", cbor_test_54 },
        { "test of cbor_test_55()", cbor_test_55 },
        { "test of cbor_test_56()", cbor_test_56 },
        { "test of cbor_test_57()", cbor_test_57 },
        { "test of cbor_test_58()", cbor_test_58 },
        { "test of cbor_test_59()", cbor_test_59 },
        { "test of cbor_test_60()", cbor_test_60 },
        { "test of cbor_test_61()", cbor_test_61 },
        { "test of cbor_test_62()", cbor_test_62 },
        { "test of cbor_test_63()", cbor_test_63 },
        { "test of cbor_test_64()", cbor_test_64 },
        { "test of cbor_test_65()", cbor_test_65 },
        { "test of cbor_test_66()", cbor_test_66 },
        { "test of cbor_test_67()", cbor_test_67 },
        { "test of cbor_test_68()", cbor_test_68 },
        { "test of cbor_test_69()", cbor_test_69 },
        { "test of cbor_test_70()", cbor_test_70 },
        { "test of cbor_test_71()", cbor_test_71 },
        { "test of cbor_test_72()", cbor_test_72 },
        { "test of cbor_test_73()", cbor_test_73 },
        { "test of cbor_test_74()", cbor_test_74 },
        { "test of cbor_test_75()", cbor_test_75 },
        { "test of cbor_test_76()", cbor_test_76 },
        { "test of cbor_test_77()", cbor_test_77 },
        { "test of cbor_test_78()", cbor_test_78 },
        { "test of cbor_test_79()", cbor_test_79 },
        { "test of cbor_test_80()", cbor_test_80 },
        { "test of cbor_test_81()", cbor_test_81 },
        { "test of cbor_test_82()", cbor_test_82 },
        { "test of cbor_test_83()", cbor_test_83 },
        { NULL, NULL },
};
